defmodule Ecto.Relation.BinaryIdDynamicRelationTest do
  ...
  describe "binary_id dynamic relations" do
    ...
    test "generates correct @primary_key attribute for binary_id", %{
      ...
      ecto_relation_schema = orgs.schema()
      assert ecto_relation_schema.source == "binary_id_organizations"

      # Verify primary key field has correct types
      pk_field = hd(ecto_relation_schema.primary_key.fields)
  ...
  describe "schema introspection" do
    ...
    test "schema introspection returns correct field information", %{
      ...
      ecto_relation_schema = users.schema()

      # Check that the schema has the correct source
      assert ecto_relation_schema.source == "binary_id_users"

      # Check primary key information
      assert length(ecto_relation_schema.primary_key.fields) == 1
      pk_field = hd(ecto_relation_schema.primary_key.fields)
      assert pk_field.name == :id
      assert pk_field.ecto_type == :binary_id

      # Check that binary_id foreign key fields are properly typed
      org_id_field = Enum.find(ecto_relation_schema.fields, &(&1.name == :organization_id))
      assert org_id_field
