defmodule Ecto.Relations.SchemaSpec do
  ...
  describe "automatic schema storage" do
    @tag relations: [:users]
    test "automatically stores Ecto.Relation.Schema", %{users: users} do
      schema = users.schema()

      # Should be a Ecto.Relation.Schema struct
      assert %Ecto.Relation.Schema{} = schema
      assert schema.source == "users"

      # Should have primary key information
      assert Ecto.Relation.Schema.PrimaryKey.field_names(schema.primary_key) == [:id]
    ...
    test "stores schema with composite primary key", %{composite_pk: composite_pk} do
      ...
      assert Ecto.Relation.Schema.PrimaryKey.field_names(schema.primary_key) == [
               :part1,
               :part2
             ]

      assert Ecto.Relation.Schema.composite_primary_key?(schema)
    end
  end
