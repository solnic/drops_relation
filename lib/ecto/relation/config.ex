defmodule Ecto.Relation.Config do
  @moduledoc """
  Configuration management for EctoRelation.

  This module provides a centralized configuration system for EctoRelation, handling
  validation, persistence, and runtime access to configuration options.

  ## Configuration

  EctoRelation can be configured through the application environment, under the `:ecto_relation` application.
  For example, you can do this in `config/config.exs`:

      # config/config.exs
      config :ecto_relation,
  ...
  Configuration for the EctoRelation schema cache.
  ...
  **Default:** `[enabled: true]`
  ...
  ## Examples
      ...
      EctoRelation.Config.put_config(:schema_cache, enabled: false)
  """
  ...
  This function reads the configuration from the `:ecto_relation` application environment
  ...
  def validate!(config) when is_list(config) do
    ...
    case NimbleOptions.validate(config_opts, @opts_schema) do
      ...
      {:error, error} ->
        raise ArgumentError, """
        invalid configuration for the :ecto_relation application, so we cannot start or update
        its configuration. The error was:

            #{Exception.message(error)}

        See the documentation for the EctoRelation.Config module for more information on configuration.
        """
    end
  end
  ...
  def persist(config) when is_list(config) do
    Enum.each(config, fn {key, value} ->
      :persistent_term.put({:ecto_relation_config, key}, value)
    end)
  end
  ...
  Once the `:ecto_relation` application starts, it validates and caches the value of the
  ...
  ## Examples
      ...
      EctoRelation.Config.update(:schema_cache, enabled: false)
  """
  ...
  defp fetch!(key) do
    :persistent_term.get({:ecto_relation_config, key})
  rescue
    ArgumentError ->
      # Fallback to application environment if persistent term not available
      Application.get_env(:ecto_relation, key, get_default(key))
  end
