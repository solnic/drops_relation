defmodule Ecto.Relation.Inference do
  alias Ecto.Relation.SQL
  alias Ecto.Relation.Schema.Field
  alias Ecto.Relation.Inference.FieldCandidate
  alias Ecto.Relation.Inference.FieldCandidates
  alias Ecto.Relation.Inference.SchemaFieldAST

  @doc """
  Builds field candidates from inferred schema, associations, and custom fields.
  """
  @spec build_field_candidates(
          Ecto.Relation.Schema.t(),
          list(),
          list()
        ) :: FieldCandidates.t()
  def build_field_candidates(ecto_relation_schema, association_definitions, custom_fields) do
    # Get primary key field names
    primary_key_names = Enum.map(ecto_relation_schema.primary_key.fields, & &1.name)
    ...
    candidates =
      Enum.reduce(ecto_relation_schema.fields, candidates, fn inferred_field, acc ->
    ...
    inferred_field_names = MapSet.new(ecto_relation_schema.fields, & &1.name)
  ...
  def generate_schema_ast_from_candidates(
    ...
    candidates =
      build_field_candidates(ecto_relation_schema, association_definitions, custom_fields)

    # Generate field definitions from candidates that should be field() calls
    # Sort by the original field order from the ecto_relation_schema to maintain consistent ordering
    field_definitions =
      candidates
      |> FieldCandidates.field_definitions()
      |> Enum.sort_by(fn candidate ->
        # Find the index of this field in the original schema fields
        Enum.find_index(ecto_relation_schema.fields, &(&1.name == candidate.field.name)) || 999
      end)
    ...
    primary_key_attr =
      generate_primary_key_attribute_from_candidates(candidates, ecto_relation_schema.primary_key)
  ...
  @spec generate_primary_key_attribute_from_candidates(
          FieldCandidates.t(),
          Ecto.Relation.Schema.PrimaryKey.t()
        ) :: Macro.t() | nil
  ...
  def infer_schema(relation, name, repo) do
    ...
    ecto_relation_schema = SQL.Inference.infer_from_table(name, repo)
    ...
    {ecto_schema_ast, ecto_relation_schema}
  end
